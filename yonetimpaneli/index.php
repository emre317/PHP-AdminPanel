...
define("SITE",$siteURL);
// session ile login sayfasında tanımlanan  ID, adsoyad,mail değerleri kontrol edilir ve başarılıysa bu sayfa çalıştırılır.
if (!empty($_SESSION["ID"]) && !empty($_SESSION["adsoyad"]) && !empty($_SESSION["mail"]))
{

}
else{ // oturum başlatılamamışsa yeniden login sayfasına yönlendirir.
    ?>
...
// eğer GET metodu kullanılmış ve sayfa adında bir parametre varsa ve boş değilse
if($_GET && !empty($_GET["sayfa"]))
{
    $sayfa=$_GET["sayfa"].".php";  // sayfa değişkeni hangi değeri alırsa alsın .php uzantılı olur.

    // sayfa değişkenine atanan dosyanın var olup olmadığı kontrol edilir.
    // eğer o sayfa varsa include edilir.
    if(file_exists(SAYFA.$sayfa))
    {
        include_once(SAYFA.$sayfa);
    }
    else{  // aksi halde yeniden anasayfaya yönlendirilir.
        include_once(SAYFA."home.php");
    }
}
// GET metodu kullanılmamıssa anasayfaya yönlendir.
else {
    include_once(SAYFA."home.php");
}
...
<script>
   ...
   // aktif pasif durumunu kontrol eden fonksiyon
function aktifpasif(ID,tablo)
{
    var durum=0;
if ($(".aktifpasif"+ID).is(':checked')) // bu bölüm seçili olarak gelmişse
{
    durum=1; // ilgili bölüm seçili gelmişse 1 ata
} else {
    durum=2; // ilgili bölüm seçili gelmemişse yani pasife alınmışsa 2 ata
}
$.ajax({
    method:"POST",  // hangi methodda gönderileceği belirtildi
    url:"<?=SITE?>ajax.php", // bu değerin hangi sayfaya gönderileceği belirtildi.
    data:{"tablo":tablo,"ID":ID,"durum":durum}, // hangi verilerin iletileceği belirtildi
    success: function (sonuc){  // dönen sonuç doğruysa
        if (sonuc=="TAMAM"){ // işlem başarılı ise aktiftir

        } else {
       // alert("İşleminiz şu an geçersizdir. Lütfen daha sonra tekrar deneyiniz.");
        }
    }
});
}

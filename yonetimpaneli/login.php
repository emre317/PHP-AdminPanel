...
<head>
  ...
  <title>YönetimPaneli | Log in</title>
...
<div class="login-box">
  <div class="login-logo">
    <a href="<?=SITE?>"><b>YönetimPaneli</b>Admini</a>
  </div>
  <!-- /.login-logo -->
  <div class="card">
    <div class="card-body login-card-body">
      <p class="login-box-msg">Giriş Yapmak İçin Bilgilerinizi Giriniz.</p>
        ...
        <?php
        if ($_POST){  // post metodu ile gönderilmişse
            if (!empty($_POST["kullanici"]) && !empty($_POST["sifre"])) // kullanıcı adı ve şifre boş değilse
            {
                $kullanici=$VT->filter($_POST["kullanici"]); // kullanıcı adı ve şifre filter metodundan geçirildi.
                $sifre=md5($VT->filter($_POST["sifre"]));
                $kontrol=$VT->VeriGetir("kullanicilar","WHERE kullanici=? AND sifre=?"   // kullanıcılar tablosundan VeriGetir fonksitonu ile değerler çekildi.
                    ,array($kullanici,$sifre),"ORDER BY ID ASC",1);
                if ($kontrol!=false)
                {
                    // oturum işlemleri kontrol edebilmek için session işlemi uygulandı.

                    ...
                    if($_POST)
                    {
                            ...
                            !empty($_POST["description"]) && !empty($_POST["sirano"]))  // ortak alanların boş bırakılmaması gerekiyor
                        {
                            ...
                            if(!empty($_FILES["resim"]["name"]))  // files nesnesi içerisinde yer alan resim alanı ve ismi boş değilse yani resim dosyası seçilmişse
                            {
                                ...
                                {
                                    $ekle=$VT->SorguCalistir("insert into ".$kontrol[0]["tablo"]," SET baslik=?, seflink=?,
                                    kategori=?, metin=?, resim=?, anahtar=?, description=?, durum=?, sirano=?, tarih=?",
                                        array($baslik,$seflink,$kategori,$metin,$yukle,$anahtar,
                                            $description,1,$sirano,date("Y-m-d"))); // EKLEME VE GÜNCELLEME İŞLEMLERİ SORGUCALISTIR FONKSIYONU ÜZERİNDEN YAPILIR
                                }
                            ...
                            {
                                $ekle=$VT->SorguCalistir(" insert into ".$kontrol[0]["tablo"]," SET baslik=?, seflink=?,
                                 kategori=?, metin=?, anahtar=?,description=?, durum=?, sirano=?, tarih=?",array($baslik,$seflink,
                                    $kategori,$metin, $anahtar,$description,1,$sirano,date("Y-m-d")));

...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            if($_POST)
            {
                // iletişim ayarları sayfasında güncelleme işlemi için gerekli verileri tanımladık
                if(  !empty($_POST["telefon"]) && !empty($_POST["mail"]) && !empty($_POST["adres"]) && !empty($_POST["fax"]))  // ortak alanların boş bırakılmaması gerekiyor
                {
                    $telefon=$VT->filter($_POST["telefon"]);
                    $mail=$VT->filter($_POST["mail"]);
                    $adres=$VT->filter($_POST["adres"]);
                    $fax=$VT->filter($_POST["fax"]);
                    $guncelle=$VT->SorguCalistir(" UPDATE ayarlar"," SET telefon=?,  mail=?,adres=?,fax=? WHERE ID=?",
                        array($telefon, $mail,$adres,$fax,1),1);

                    if($guncelle!=false){   // ekleme işlemi başarılıysa çıkan uyarı mesajı ve yönlendrilecek sayfa belirlendi
                        ?>
